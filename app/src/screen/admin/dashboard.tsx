...
export default function AdminDashboard() {
    const { data, query, refetch, isLoading: loading } = useQuery({
        callback: async () => await MachineService.getMachineState()
    });

    const { data: processes, query: queryProcesses, refetch: refetchProcesses, isLoading: isLoadingProcesses, isPending: isPendingProcess } = useQuery({
    ...
    const parseProcessData = (processes: string[]) => {
        ...
        return processes.map((process, index) => {
            ...
            const parts = process.trim().split('|').map(part => part.trim());
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <Card className="p-6 hover:shadow-xl transition-all duration-300">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide">Hostname</p>
                    ...
                    <Card className="p-6 hover:shadow-xl transition-all duration-300">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide">CPU Cores</p>
                    ...
                    <Card className="p-6 hover:shadow-xl transition-all duration-300">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-foreground mt-1">{machineData?.uptime && formatUptime(machineData.uptime)}</p>
                            </div>
                    ...
                    <Card className="p-6 hover:shadow-xl transition-all duration-300">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wide">Memory Usage</p>
                                <p className="text-2xl font-bold text-foreground mt-1">{machineData?.memoryUsage?.toFixed(1)}%</p>
                            </div>
                            <div className="p-3 rounded-full bg-primary text-primary">
                                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Card className="p-8 hover:shadow-xl transition-all duration-300">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center py-3 border-b border-muted">
                                ...
                                <span className="font-semibold text-foreground bg-muted px-3 py-1 rounded-full text-sm">{machineData?.platform}</span>
                            </div>
                            <div className="flex justify-between items-center py-3 border-b border-muted">
                                <span className="text-muted-foreground font-medium">Platform Family</span>
                                <span className="font-semibold text-foreground bg-muted px-3 py-1 rounded-full text-sm">{machineData?.platformFamily}</span>
                            </div>
                            <div className="flex justify-between items-center py-3 border-b border-muted">
                                <span className="text-muted-foreground font-medium">Platform Version</span>
                                <span className="font-semibold text-foreground bg-muted px-3 py-1 rounded-full text-sm">{machineData?.platformVersion}</span>
                            </div>
                            <div className="flex justify-between items-center py-3 border-b border-muted">
                                <span className="text-muted-foreground font-medium">Kernel Version</span>
                                <span className="font-semibold text-foreground bg-muted px-3 py-1 rounded-full text-sm">{machineData?.kernelVersion}</span>
                            </div>
                            <div className="flex justify-between items-center py-3">
                                <span className="text-muted-foreground font-medium">Architecture</span>
                                <span className="font-semibold text-foreground bg-muted px-3 py-1 rounded-full text-sm">{machineData?.kernelArch}</span>
                            </div>
                        </div>
                    </Card>
                        ...
                        <div className="space-y-6">
                            <div>
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <span className="font-bold text-2xl text-foreground">{machineData?.memoryUsage?.toFixed(1)}%</span>
                                </div>
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <div className="bg-muted rounded-xl p-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-bold text-foreground text-lg">{machineData?.totalMemory && formatBytes(machineData.totalMemory)}</span>
                                    </div>
                                </div>
                                <div className="bg-muted rounded-xl p-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-muted-foreground font-medium">Used Memory</span>
                                        <span className="font-bold text-foreground text-lg">{machineData?.usedMemory && formatBytes(machineData.usedMemory)}</span>
                                    </div>
                                </div>
                                <div className="bg-muted rounded-xl p-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-muted-foreground font-medium">Available Memory</span>
                                        <span className="font-bold text-foreground text-lg">{machineData?.availableMemory && formatBytes(machineData.availableMemory)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
